...
const WGLMR_USDC_PAIR = '0x5E3ebf3813Ece2CE3090C2Ce36cC567dD0b279FD'
...
class PriceModel {
    constructor(dataseed, whitelist, reserveCurrency, reserveUsdPair, factoryAddress, minimiumLiquidity, usdDecimals) {
    ...
    async sync(blockNumber) {
        ...
        try {
            ...
            const factoryContract = new ethers.Contract(this.factoryAddress, PADSWAP_FACTORY_ABI, this.provider)
            ...
            for (let i = 0; i < tokens.length; i++) {
                const tokenPairs = pairInfos.filter(p => [p.token0.toLowerCase(), p.token1.toLowerCase()].includes(tokens[i]))
    ...
    _getTokens() {
        return new List(this.pairInfos).selectMany(p => [p.token0, p.token1])
    ...
    _getTokenGraph() {
        ...
        const maxDecimals = new List(this.pairInfos).selectMany(p => [p.token0Decimals, p.token1Decimals])
                                                    .max()
        for (const {
            token0,
            token1,
            ...
            if ((token0 == this.reserveCurrency && reserve0.lt(this.minimiumLiquidity)) ||
                (token1 == this.reserveCurrency && reserve1.lt(this.minimiumLiquidity))) {
                continue
            }

            if (!this.whitelist.includes(token0.toLowerCase()) && !this.whitelist.includes(token1.toLowerCase())) {
                continue
            }
            ...
            const zeroToOneExchangeRate = { numerator: reserve1Normalized, denominator: reserve0Normalized }
            const oneToZeroExchangeRate = { numerator: reserve0Normalized, denominator: reserve1Normalized }
