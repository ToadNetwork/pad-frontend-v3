<template>
  <v-container class="padswap-farms-home">
    <div class="padswap-header-box">
      ...
      <div style="font-size: 14px; color: #B3B8C1; margin-top: 10px;">Manually update the reward pools</div>
    </div>
        ...
        >
          <v-card-title>
            <v-btn
              color="primary"
              @click="mintPadForAll()">
                Mint PAD for all farms on selected chain
              </v-btn>
          </v-card-title>
          <v-card-subtitle style="margin-top: 5px;">
            (this will initiate a bunch of transactions, you will need to approve each one)
          </v-card-subtitle>
...
export default Vue.extend({
  name: 'Home',
  mixins: [formatMixin],
  components: { FarmManager, SliderTabs },
  data() {
  ...
  methods: {
    ...
    async mintPadForAll() {
      ...
      for (const farm of allFarms) {
        ...
        if (farm.type != FarmType.LP) {
          const tx = await farmContract.populateTransaction.mintPad()
          this.safeSendTransaction({ tx, targetChainId: this.ecosystem.chainId})
        }
      }
    },
